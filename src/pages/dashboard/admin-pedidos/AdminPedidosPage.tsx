...
export default function AdminPedidosPage() {
  ...
  useEffect(() => {
    if (!profile || !['recepcionista', 'veterinario', 'administrador', 'superadmin'].includes(profile.role)) return
    fetchPedidos()
  }, [profile])
  ...
  const fetchPedidos = async () => {
    const { data } = await supabase
      .from('pedidos')
      .select('id, estado, tipo_entrega, direccion_envio, notas, fecha_pedido, items:pedido_items(cantidad, precio_unitario, producto:productos(nombre)), perfil:profiles!cliente_id(nombre_completo, telefono)')
        ...
        return (
          <div key={pedido.id} className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div className="flex gap-2">
              {puedeRecibir && (
                <button onClick={() => actualizarEstado(pedido.id, 'recibido')} className="bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 text-sm">
                  ✅ Recibido
                </button>
              )}
              {puedeEnviar && (
                <button onClick={() => actualizarEstado(pedido.id, 'enviado_entrega')} className="bg-blue-600 text-white px-4 py-2 rounded-xl hover:bg-blue-700 text-sm">
                  🚀 Enviado
                </button>
              )}
              {puedeEntregar && (
                <button onClick={() => actualizarEstado(pedido.id, 'entregado')} className="bg-purple-600 text-white px-4 py-2 rounded-xl hover:bg-purple-700 text-sm">
                  📬 Entregado
                </button>
              )}
            </div>
