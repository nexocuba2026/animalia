...
export default function CitasPage() {
  ...
  const confirmarReagendar = async (id: string) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', id)
    if (!error) {
      setCitas((prev) => prev.map((c) => (c.id === id ? { ...c, fecha_hora_asignada: nuevaFecha } : c)))
    }
        ...
        <section>
          ...
          <div className="grid gap-4">
            {citasHistorial.map((cita) => (
              <div key={cita.id} className={`p-4 rounded-xl shadow border ${
                cita.estado === 'realizada' ? 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800' :
                cita.estado === 'no_realizada' ? 'bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800' :
                ...
                <div className="flex justify-between items-start">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                    ...
                    cita.estado === 'no_realizada' ? 'bg-red-100 text-red-700' :
