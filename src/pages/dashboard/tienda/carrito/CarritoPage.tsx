...
export default function CarritoPage() {
  ...
  const confirmarPedido = async () => {
    ...
    if (error) {
      setMensaje('Error al confirmar: ' + error.message)
      setEnviando(false)
    } else {
      setMensaje('✅ Pedido confirmado. Redirigiendo...')
      setTimeout(() => navigate('/dashboard/pedidos'), 2000)
    }
  }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          ...
          <Link to="/dashboard/tienda" className="text-orange-600 hover:underline mt-2 block">
            Ir a la tienda
          </Link>
        </div>
      ) : (
        <>
          ...
          {mensaje && (
            <div className={`p-4 rounded-xl ${mensaje.startsWith('Error') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
              {mensaje}
            </div>
          )}
          <button
            ...
            className="w-full bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
